...
class PagespeedTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PagespeedTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PagespeedTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PagespeedTestRunner.getenv("PAGESPEED_TEST_LIVE")
        override = PagespeedTestRunner.getenv("PAGESPEED_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PagespeedTestRunner.getenv(key)
        ...
        explain = PagespeedTestRunner.getenv("PAGESPEED_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PAGESPEED_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PagespeedTestRunner._test_control is not None:
            return PagespeedTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PagespeedTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PagespeedTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PagespeedTestRunner.load_test_control()
...
def load_env_local():
    PagespeedTestRunner.load_env_local()


def env_override(m):
    return PagespeedTestRunner.env_override(m)


def entity_data(v):
    return PagespeedTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PagespeedTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PagespeedTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PagespeedTestRunner.load_test_control()


def live_client_options():
    return PagespeedTestRunner.live_client_options()


def live_delay_ms():
    return PagespeedTestRunner.live_delay_ms()
