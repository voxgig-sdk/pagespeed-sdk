...
class TestRunPagespeedEntity:

    def test_should_create_instance(self):
        testsdk = PagespeedSDK.test(None, None)
        ent = testsdk.RunPagespeed(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _run_pagespeed_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "run_pagespeed." + _op, "live" if _live else "unit")
            if _skip:
...
def _run_pagespeed_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/run_pagespeed/RunPagespeedTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PAGESPEED_TEST_RUN_PAGESPEED_ENTID"))
    ...
    if env.get("PAGESPEED_TEST_LIVE") == "TRUE":
        ...
        client = PagespeedSDK(helpers.to_map(merged_opts))

    _live = env.get("PAGESPEED_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("PAGESPEED_TEST_EXPLAIN") == "TRUE",
        "live": _live,
