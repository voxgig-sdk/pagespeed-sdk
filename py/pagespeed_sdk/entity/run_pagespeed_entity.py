...
from pagespeed_sdk.pagespeed_types import (
    RunPagespeed,
...
class RunPagespeedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RunPagespeedEntity(self._client, opts)
    ...
    def data_get(self) -> RunPagespeed:
    ...
    def match_get(self) -> RunPagespeed:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RunPagespeed:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RunPagespeed().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
