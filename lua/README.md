...
The Lua SDK for the Pagespeed API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:RunPagespeed()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Create an instance: `local run_pagespeed = client:RunPagespeed(nil)`
...
```
lua/
├── pagespeed_sdk.lua    -- Main SDK module
...
The main module (`pagespeed_sdk`) exports the SDK constructor
