...
describe("RunPagespeedEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RunPagespeed(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = run_pagespeed_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "run_pagespeed." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set PAGESPEED_TEST_RUN_PAGESPEED_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local run_pagespeed_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.run_pagespeed")))
    local run_pagespeed_ref01_data = nil
    if #run_pagespeed_ref01_data_raw > 0 then
      run_pagespeed_ref01_data = helpers.to_map(run_pagespeed_ref01_data_raw[1][2])
    end

    -- LOAD
    local run_pagespeed_ref01_ent = client:RunPagespeed(nil)
    local run_pagespeed_ref01_match_dt0 = {
...
function run_pagespeed_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/run_pagespeed/RunPagespeedTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read run_pagespeed test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("PAGESPEED_TEST_RUN_PAGESPEED_ENTID")
  ...
  if env["PAGESPEED_TEST_LIVE"] == "TRUE" then
