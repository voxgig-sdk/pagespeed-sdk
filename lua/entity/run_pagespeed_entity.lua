...
local RunPagespeedEntity = {}
RunPagespeedEntity.__index = RunPagespeedEntity


function RunPagespeedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RunPagespeedEntity)
  self._name = "run_pagespeed"
  self._client = client
...
function RunPagespeedEntity:get_name()
  return self._name
end


function RunPagespeedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RunPagespeedEntity.new(self._client, opts)
end
...
function RunPagespeedEntity:mark_deleted()
  self._deleted = true
end


function RunPagespeedEntity:deleted()
  return true == self._deleted
end


function RunPagespeedEntity:data_set(args)
  if args ~= nil then
...
function RunPagespeedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RunPagespeedEntity:match_set(args)
  if args ~= nil then
...
function RunPagespeedEntity:match_get()
...
function RunPagespeedEntity:stream(action, args, callopts)
...
---@param reqmatch RunPagespeedLoadMatch
---@param ctrl? table
---@return RunPagespeed
---@return string? err
function RunPagespeedEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RunPagespeedEntity:_run_op(ctx, post_done)
  local utility = self._utility
