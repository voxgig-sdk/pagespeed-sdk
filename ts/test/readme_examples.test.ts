...
//     (\`new PagespeedSDK(...)\` / \`PagespeedSDK.test(...)\` become
//     \`PagespeedSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"run_pagespeed":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PagespeedSDK(...)\` and any
// \`PagespeedSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PagespeedSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PagespeedSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
