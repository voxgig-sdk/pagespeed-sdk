...
//
// NewPagespeedSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.PagespeedSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewPagespeedSDK(opts)
}
...
func fcCandidates(client *sdk.PagespeedSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.PagespeedSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.PagespeedSDK, name string) any {
