...
import (
	...
	omni "github.com/voxgig-sdk/pagespeed-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PagespeedError); is {
				out := map[string]any{
					"name":    "PagespeedError",
...
func omniCtx(arg any, client *sdk.PagespeedSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PagespeedSDK); is {
			client = live
