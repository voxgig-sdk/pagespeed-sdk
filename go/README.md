# Pagespeed Golang SDK



The Golang SDK for the Pagespeed API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/pagespeed-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/pagespeed-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/pagespeed-sdk/go=../pagespeed-sdk/go
```
...
```go
runpagespeed, err := client.RunPagespeed(nil).Load(map[string]any{"url": "example"}, nil)
if err != nil {
    // handle err
    return
}
...
```
PAGESPEED_TEST_LIVE=TRUE
PAGESPEED_APIKEY=<your-key>
```
...
```go
func NewPagespeedSDK(options map[string]any) *PagespeedSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *PagespeedSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### PagespeedSDK methods
...
| `RunPagespeed` | `(data map[string]any) PagespeedEntity` | Create a RunPagespeed entity instance. |

### Entity interface (PagespeedEntity)

All entities implement the `PagespeedEntity` interface.
...
slice):
    ...
    runPagespeed, err := client.RunPagespeed(nil).Load(nil, nil)
    if err != nil { /* handle */ }
...
```go
runPagespeed, err := client.RunPagespeed(nil).Load(map[string]any{"url": "url"}, nil)
if err != nil {
    panic(err)
}
...
```
github.com/voxgig-sdk/pagespeed-sdk/go/
├── pagespeed.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/pagespeed-sdk/go`) re-exports everything needed
...
runpagespeed.Load(map[string]any{"url": "example"}, nil)

// runpagespeed.Data() now returns the runpagespeed data from the last load
// runpagespeed.Match() returns the last match criteria
```
