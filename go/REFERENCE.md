# Pagespeed Golang SDK Reference

Complete API reference for the Pagespeed Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *PagespeedSDK`
...
#### `RunPagespeed(data map[string]any) PagespeedEntity`
...
```go
result, err := client.RunPagespeed(nil).Load(map[string]any{"url": "url"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
