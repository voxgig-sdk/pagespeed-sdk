...
class PagespeedSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PagespeedHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PagespeedHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PagespeedFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PagespeedFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PagespeedUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PagespeedHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PagespeedHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PagespeedHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PagespeedError($op . "_allow",
                "PagespeedSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PagespeedHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PagespeedError("graphql_error",
                "PagespeedSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function RunPagespeed($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_run_pagespeed === null) {
                $this->_run_pagespeed = new RunPagespeedEntity($this, null);
            }
            return $this->_run_pagespeed;
        }
        return new RunPagespeedEntity($this, $data);
    }
