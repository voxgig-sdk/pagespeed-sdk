...
class PagespeedNetsimFeature extends PagespeedBaseFeature
{
    ...
    private function _simulate(PagespeedContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(PagespeedContext $ctx, array $applied): void
    {
        $client = $this->client;
