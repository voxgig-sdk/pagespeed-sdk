...
class PagespeedMakeError
{
    public static function call(?PagespeedContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PagespeedContext([], null);
        }
        $op = $ctx->op ?? new PagespeedOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PagespeedResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PagespeedError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PagespeedError('', $msg, $ctx);
