...
{
    private static function utility(): PagespeedUtility
    {
        return new PagespeedUtility();
    }
    ...
    private static function ctx(array $over = []): PagespeedContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PagespeedContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PagespeedOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PagespeedError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PagespeedOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PagespeedMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PagespeedOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PagespeedMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PagespeedOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PagespeedMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PagespeedSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PagespeedMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PagespeedResponse([]),
            'result' => new PagespeedResult([]),
        ]);
        [, $err] = PagespeedMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PagespeedSpec([]),
            'response' => null,
            'result' => new PagespeedResult([]),
        ]);
        [, $err] = PagespeedMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PagespeedSpec(['step' => 's']),
            'response' => new PagespeedResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PagespeedResult([]),
        ]);
        [, $err] = PagespeedMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PagespeedSpec(['step' => 's']),
            'response' => new PagespeedResponse(self::resp(200, ['v' => 1])),
            'result' => new PagespeedResult([]),
        ]);
        [, $err] = PagespeedMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PagespeedSpec(['step' => 's']),
            'response' => new PagespeedResponse(self::resp(200, ['v' => 2])),
            'result' => new PagespeedResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PagespeedMakeResponse::call($ctx);
    ...
    {
        $preset = new PagespeedResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PagespeedSpec([]),
            'response' => new PagespeedResponse([]),
            'result' => new PagespeedResult([]),
        ...
        [$got, $err] = PagespeedMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PagespeedResult([])]);
        [, $err] = PagespeedMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PagespeedSpec([]), 'result' => null]);
        [, $err] = PagespeedMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PagespeedSpec(['step' => 's']),
            'result' => new PagespeedResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PagespeedOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PagespeedMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PagespeedSpec(['step' => 's']),
            'result' => new PagespeedResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PagespeedOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PagespeedMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PagespeedResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PagespeedSpec([]),
            'result' => new PagespeedResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PagespeedMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PagespeedMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PagespeedError('boom', 'boom');
        $utility->fetcher = function (PagespeedContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PagespeedSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PagespeedMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PagespeedContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PagespeedMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PagespeedResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PagespeedContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PagespeedSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PagespeedMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PagespeedResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PagespeedSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PagespeedMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PagespeedMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PagespeedMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PagespeedResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PagespeedDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PagespeedResult(['ok' => false])]);
        $this->expectException(PagespeedError::class);
        PagespeedDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PagespeedResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PagespeedMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PagespeedMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PagespeedBaseFeature();
        $b = new PagespeedBaseFeature();
        PagespeedFeatureAdd::call($ctx, $a);
        PagespeedFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PagespeedBaseFeature
    {
        $f = new PagespeedBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PagespeedFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PagespeedContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PagespeedSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PagespeedPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PagespeedResponse(['headers' => null]),
            'result' => new PagespeedResult([]),
        ]);
        PagespeedResultHeaders::call($ctx);
