...
class FtCtrl extends PagespeedControl
{
...
class FtClient
{
    ...
    private PagespeedUtility $utility;

    public function __construct(PagespeedUtility $utility, array $options)
    {
    ...
    public function get_utility(): PagespeedUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PagespeedContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PagespeedContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PagespeedContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PagespeedConfig::make_feature($name);
    ...
    private static function buildUrl(PagespeedSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PagespeedError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PagespeedResponse($fetched);
        }
    ...
    private function fail(PagespeedContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PagespeedContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
