...
boru-driven command-line client **and** interactive REPL for the Pagespeed
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pagespeed-cli)
make build
...
./pagespeed-cli load 1 run_pagespeed            # {id:1} shorthand
./pagespeed-cli load '{id:1}' run_pagespeed       # explicit match map

# 5. Override the API base URL for a single call
PAGESPEED_BASE=https://api.example.com ./pagespeed-cli load 1 run_pagespeed

# 6. No arguments -> interactive REPL
./pagespeed-cli
pagespeed> load 1 run_pagespeed
pagespeed> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pagespeed-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pagespeed-cli load 1 run_pagespeed
   ```
...
```sh
./pagespeed-cli load 1 run_pagespeed          # scalar shorthand for {id:1}
./pagespeed-cli load '{id:1}' run_pagespeed     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pagespeed>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `run_pagespeed`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pagespeed-cli`. |
...
(`./pagespeed-cli <expr>`) and interactively (the REPL), and expressions compose the
