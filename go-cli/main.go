// Command Pagespeed-cli is a CLI + REPL for the Pagespeed SDK,
...
const prompt = "pagespeed"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "run_pagespeed"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PAGESPEED_APIKEY (api key), PAGESPEED_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PAGESPEED_APIKEY carries the API key and
	// PAGESPEED_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PAGESPEED_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PAGESPEED_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPagespeedSDK(opts)
