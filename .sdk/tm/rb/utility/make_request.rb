...
module PagespeedUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PagespeedResponse.new({})
    ...
    if fetch_err
      ...
      response = PagespeedResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PagespeedResponse.new(fetched)
    else
