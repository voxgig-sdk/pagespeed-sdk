...
require_relative 'Pagespeed_types'
...
class PagespeedSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PagespeedHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PagespeedHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PagespeedFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PagespeedUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PagespeedHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PagespeedHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PagespeedHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PagespeedHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PagespeedError.new(
        "graphql_error", "PagespeedSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
