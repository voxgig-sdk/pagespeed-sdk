...
module PagespeedUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PagespeedContext.new({}, nil)
    end
    op = ctx.op || PagespeedOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PagespeedResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PagespeedError) ? err.msg : err.to_s
    ...
    sdk_err = PagespeedError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PagespeedError)
