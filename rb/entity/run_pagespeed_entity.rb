...
class RunPagespeedEntity
  ...
  def make
    opts = @_entopts.dup
    RunPagespeedEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PagespeedHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PagespeedHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PagespeedHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
