...
MCP server exposing the Pagespeed SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `pagespeed_list` | `entity`, optional `query` map | First page of records as JSON |
| `pagespeed_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `pagespeed_list` and
`pagespeed_load` tools become available in new sessions.
