...
[MCP](https://modelcontextprotocol.io) server exposing the Pagespeed SDK as
two agent tools — `pagespeed_list` and `pagespeed_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pagespeed-mcp)
make build

# 2. Provide credentials via the environment
export PAGESPEED_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pagespeed \
  -- /absolute/path/to/pagespeed-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pagespeed-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pagespeed_list: first page of records
{ "entity": "run_pagespeed" }
{ "entity": "run_pagespeed", "query": { } }

// pagespeed_load: one record by id
{ "entity": "run_pagespeed", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pagespeed-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pagespeed \
     -- "$PWD"/dist/*/pagespeed-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pagespeed_list` and `pagespeed_load` tools now appear
   in new sessions. Ask the agent to *"list run_pagespeed using pagespeed"*
   and it calls `pagespeed_list` with `{"entity":"run_pagespeed"}`.
...
```sh
export PAGESPEED_APIKEY=sk_live_xxx            # API key
export PAGESPEED_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pagespeed_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pagespeed_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PAGESPEED_APIKEY` | API key sent with every request. |
| `PAGESPEED_BASE` | Optional override of the API base URL. |
...
```sh
./pagespeed-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pagespeed_load","arguments":{"entity":"run_pagespeed","query":{"id":1}}}}'
```
